...
// Self-contained copy of the JIT26 breakpoint-protocol client primitives for FEXCore's own,
// independently-linked static library. The canonical, live-device-tested implementation lives in
// tools/sogen-ios/Sources/JIT/JIT26.c, part of the SogenIOS app target; libFEXCore.a is built as a
// completely separate static library with no link-time visibility into that target's sources, so
// it needs its own copy of these symbols rather than resolving them across a library boundary it
// cannot cross. See src/common/utils/ios_device_jit_mmap_shim.cpp for the same precedent applied
// to Unicorn's own backend.
